...
#    global global_data
    ...
    if not staubli.GetManipulators()[0].GetIkSolver():
        ikmodel = openravepy.databases.inversekinematics.InverseKinematicsModel(staubli,iktype=IkParameterization.Type.Transform6D)
    
        if not ikmodel.load(roslib.packages.get_pkg_dir('trajectory_planner') +'/ikfast_model/ikfast41.Transform6D.x86_64.0_1_2_3_4_5.so'):
            ikmodel.autogenerate()
            rospy.loginfo("IkModel autogenerated")
        else:
            rospy.loginfo("IKModel loaded from trajectory_planner package")
    else:
        rospy.loginfo("IkModel found in database")
